...
def envelope(items: Any) -> dict[str, Any]:
    return {"items": items, "asOf": datetime.now(timezone.utc).date().isoformat(),
            "snapshotId": f"{MODE.lower()}-snapshot", "qualityStatus": "REFERENCE",
            "mode": MODE, "sourceType": "AKSHARE_PUBLIC",
            "trustLevel": "LOW"}
...
def filtered(request: ScreenRequest) -> list[Fund]:
    ...
    for fund in repository.list_funds():
        ...
        if filters.fundTypes and fund.type not in filters.fundTypes:
            continue
        if filters.riskLevelMax and fund.risk in RISK_RANK and RISK_RANK[fund.risk] > RISK_RANK.get(filters.riskLevelMax, 99):
            continue
        if filters.maxFee and fund.fee is not None and fund.fee > filters.maxFee:
            continue
        if filters.requireOpen and fund.intake not in {"开放申购", "未获取"}:
            continue
        if filters.minimumInceptionYears and fund.inception is not None and fund.inception < filters.minimumInceptionYears:
            continue
...
async def health() -> dict[str, Any]:
    return {"status": "ok", "mode": MODE.lower(), "provider": "python-fastapi", "dataSource": provider.status.as_dict()}
